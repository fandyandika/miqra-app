...
import { myFamilies, familyMembers, createFamily, createInvite, redeemInvite } from '@/services/family';

export function useMyFamilies() {
  return useQuery({ queryKey: ['families','mine'], queryFn: myFamilies, staleTime: 60_000 });
}
...
export function useCreateInvite() {
  return useMutation({ mutationFn: ({ familyId, ttlMinutes }:{familyId:string; ttlMinutes?:number}) => createInvite(familyId, ttlMinutes) });
}
