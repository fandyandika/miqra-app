    ...
    <View className="flex-1 bg-background px-5 pt-14">
      ...
      {statsLoading ? (
        ...
        <View className="items-center mb-6 p-6 bg-red-50 rounded-xl">
          <Text className="text-red-800 text-center mb-2">❌ Error memuat data keluarga</Text>
        ...
        <View className="items-center mb-6">
          ...
          <Text className="mt-3 text-charcoal font-medium text-center">
            {statsData.membersReadToday} dari {statsData.totalMembers} anggota sudah membaca hari
            ini
          </Text>
      ...
      {membersQ.isLoading ? (
        ...
        <View className="mt-4">
          <FlatList
            ...
            renderItem={({ item }) => {
              const displayName =
                item.profiles?.[0]?.display_name || `User ${item.user_id.slice(0, 6)}`;
              const initial = displayName.charAt(0).toUpperCase();

              // Check if this member has read today
              const hasReadToday =
                statsData?.members?.find((m) => m.name === displayName)?.readToday || false;
