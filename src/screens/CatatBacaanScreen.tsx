...
export default function CatatBacaanScreen() {
  ...
  const [range, setRange] = React.useState({
    start: progress?.current_ayat ?? 1,
    end: progress?.current_ayat ?? 1,
  });
  ...
  function submit() {
    console.log('[CatatBacaan] Submit called with:', { surah, range, notes });

    if (!surah || !range.start || !range.end) {
      console.log('[CatatBacaan] Validation failed: missing required fields');
      return;
    }
    ...
    if (range.end > max) {
      console.log('[CatatBacaan] Validation failed: end > max', {
        end: range.end,
        max,
      });
    ...
    console.log('[CatatBacaan] Calling mutation...');
    m.mutate({
      surah_number: surah,
