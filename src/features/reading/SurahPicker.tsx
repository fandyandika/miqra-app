    ...
    <View>
      ...
      <Modal visible={open} animationType="slide">
        <View style={{ flex:1, padding:16 }}>
          <Pressable onPress={()=>setOpen(false)}><Text>← Kembali</Text></Pressable>
          <TextInput
            placeholder="Cari surah..."
            value={q}
            onChangeText={setQ}
            style={{ borderWidth:1, borderColor:'#E5E7EB', borderRadius:8, padding:8, marginVertical:12 }}
          />
          <FlatList
            data={filtered}
            keyExtractor={(s)=>String(s.number)}
            renderItem={({item})=> (
              <Pressable onPress={()=>{ onChange(item.number); setOpen(false); }} style={{ paddingVertical:10 }}>
                <Text>{item.number}. {item.name} ({item.ayatCount})</Text>
              </Pressable>
            )}
          />
        </View>
