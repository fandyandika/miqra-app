...
export async function getFamilyTodayStats(familyId: string, timezone = 'Asia/Jakarta') {
  ...
  if (memberIds.length > 0) {
    ...
    if (profilesErr) {
      memberList = memberIds.map(id => ({ id, name: `User ${id.slice(0,6)}` }));
    } else {
  ...
  if (memberIds.length === 0)
    return { totalMembers: 0, membersReadToday: 0, familyStreakDays: 0, members: [] };
  ...
  const distinctDates = Array.from(new Set(checkins.map((c) => c.date))).sort().reverse();
  let streak = 0;
