// Test script for ReaderScreen component
// Run with: npx tsx scripts/test-reader-screen.ts
...
// Mock createReadingSession function
async function mockCreateReadingSession(input: any) {
  console.log('📝 Mock createReadingSession called with:', input);
  return { id: 'mock-session-id', ...input };
}
...
async function testReaderScreen() {
  ...
  try {
    const result = await mockCreateReadingSession(testSession);
